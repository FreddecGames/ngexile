...
class View(BaseView):
        ...
        if cat == 1:
            ...
            if self.oAllianceRights["can_kick_player"]:
                if action == "kick":
                    ...
                    oConnExecute("SELECT sp_alliance_kick_member("+str(self.profile['id'])+","+strSql(self.username)+")")
        
        if cat == 2 and self.username != "":
            if self.oAllianceRights["can_invite_player"]:

                oRs = oConnExecute("SELECT sp_alliance_invite(" + str(self.profile['id']) + "," + strSql(self.username) + ")")
                if oRs[0] == 0:
    ...
    def displayMembers(self, content):
        ...
        # list members
        query = "SELECT username, CASE WHEN id="+str(self.profile['id'])+" OR score_visibility >=1 THEN score ELSE 0 END AS score, int4((SELECT count(1) FROM nav_planet WHERE ownerid=users.id)) AS colonies," + \
        ...
        for oRs in oRss:
            ...
            if oRs[11] >= 1 or oRs[8] == self.profile['id']:
            ...
            if self.oAllianceRights["leader"]:
                if oRs[5] > self.AllianceRank or oRs[8] == self.profile['id']:
                    item["manage"] = True
                else:
    ...
    def displayPage(self, cat):
        ...
        content = getTemplateContext(self.request, "alliance-members")
    ...
    def SaveRanks(self):
        ...
        for oRs in oRss:
            query = " UPDATE users SET" + \
                    ...
                    " WHERE id=" + str(oRs[0]) + " AND alliance_id=" + str(self.AllianceId) + " AND (alliance_rank > 0 OR id=" + str(self.profile['id']) + ")"
            dbQuery(query)

        # if leader demotes himself
        if toInt(self.request.POST.get("player" + str(self.profile['id'])), 100) > 0: return HttpResponseRedirect("/s03/alliance-view/")
