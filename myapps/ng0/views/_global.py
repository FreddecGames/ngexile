...
class GlobalView(BaseView):
    ...
    def pre_dispatch(self, request, *args, **kwargs):
        ...
        query = ' SELECT username, privilege, resets, credits, lastplanetid, deletion_date, planets, score, previous_score, mod_planets, mod_commanders,' + \
                ...
                ' (SELECT int4(COUNT(*)) FROM messages WHERE ownerid=' + str(self.userId) + ' AND read_date is NULL) AS new_mail_count,' + \
                ' (SELECT int4(COUNT(*)) FROM reports WHERE ownerid=' + str(self.userId) + ' AND read_date is NULL AND datetime <= now()) AS new_report_count' + \
                ' FROM users' + \
                ' WHERE id=' + str(self.userId)
        self.profile = dbRow(query)
        
        if self.profile['privilege'] == -3 and self.profile['resets'] == 0: return HttpResponseRedirect('/ng0/home-start/')
        elif self.profile['privilege'] == -3: return HttpResponseRedirect('/ng0/home-wait/')
        elif self.profile['privilege'] == -2: return HttpResponseRedirect('/ng0/home-holidays/')        
        elif self.profile['planets'] <= 0 or self.profile['credits_bankruptcy'] <= 0: return HttpResponseRedirect('/ng0/home-gameover/')
