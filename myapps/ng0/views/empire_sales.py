...
class View(GetPostView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.tpl = getTemplate(request, "ng0/market-sell")
        ...
        query = "SELECT id, name, galaxy, sector, planet, ore, hydrocarbon, ore_capacity, hydrocarbon_capacity, planet_floor," + \
                ...
                " ORDER BY v.id"
        planets = dbRows(query)
        self.tpl.setValue("m_planets", planets)
        ...
        if get_planet != "":
            ...
            self.headerUrl = '/ng0/market-sell/'
            
            self.tpl.setValue("showHeader", self.showHeader)
            
        else:
        
            self.selectedMenu = "merchants"
        
            self.tpl.setValue("total", int(total))
        
        #---
        
        if count > 0: self.tpl.Parse("sell")
