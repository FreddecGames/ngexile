...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if action == "accept":
            oRs = oConnExecute("SELECT sp_alliance_accept_invitation(" + str(self.profile['id']) + "," + strSql(alliance_tag) + ")")
            ...
            oConnExecute("SELECT sp_alliance_decline_invitation(" + str(self.profile['id']) + "," + strSql(alliance_tag) + ")")
        elif action == "leave":
            if request.POST.get("leave", "") == "1":
                oRs = oConnExecute("SELECT sp_alliance_leave(" + str(self.profile['id']) + ",0)")
                if oRs[0] == 0:
                    return HttpResponseRedirect("/s03/alliance-view/")

        content = getTemplateContext(self.request, "alliance-invitations")
        ...
        query = "SELECT alliances.tag, alliances.name, alliances_invitations.created, users.username" + \
                ...
                " WHERE userid=" + str(self.profile['id']) + " AND NOT declined" + \
