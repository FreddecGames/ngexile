...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        oConnExecute("SELECT sp_update_researches(" + str(self.profile['id']) + ")")
        ...
        if ResearchId != 0:
            ...
            if Action == "research":
                ...
                dbQuery("UPDATE researches_pending SET looping=true WHERE userid=" + str(self.profile['id']) + " AND researchid=" + str(ResearchId))
            elif Action == "stop":
                dbQuery("UPDATE researches_pending SET looping=false WHERE userid=" + str(self.profile['id']) + " AND researchid=" + str(ResearchId))
    ...
    def ListResearches(self):
        ...
        oRs = oConnExecute("SELECT int4(count(1)) FROM researches_pending WHERE userid=" + str(self.profile['id']) + " LIMIT 1")
        ...
        query = "SELECT researchid, category, total_cost, total_time, level, levels, researchable, buildings_requirements_met, status," + \
                " (SELECT looping FROM researches_pending WHERE researchid = t.researchid AND userid=" + str(self.profile['id']) + ") AS looping," + \
                " expiration_time IS NOT NULL" + \
                " FROM sp_list_researches(" + str(self.profile['id']) + ") AS t" + \
                " WHERE level > 0 OR (researchable AND planet_elements_requirements_met)"
        oRss = oConnExecute(query)
        
        content = getTemplateContext(self.request, "empire-techs")

        content.assignValue("userid", self.profile['id'])
    ...
    def StartResearch(self, ResearchId):
        oConnExecute("SELECT * FROM sp_start_research(" + str(self.profile['id']) + ", " + str(ResearchId) + ", false)")

    def CancelResearch(self, ResearchId):
        oConnExecute("SELECT * FROM sp_cancel_research(" + str(self.profile['id']) + ", " + str(ResearchId) + ")")
