...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if action == "pay":
            ...
            oRs = oConnExecute("SELECT sp_alliance_war_pay_bill(" + str(self.profile['id']) + "," + strSql(self.tag) + ")")
            ...
            oRs = oConnExecute("SELECT sp_alliance_war_stop(" + str(self.profile['id']) + "," + strSql(self.tag) + ")")
            ...
            oRs = oConnExecute("SELECT sp_alliance_war_declare(" + str(self.profile['id']) + "," + strSql(self.tag) + ")")
            if oRs[0] == 0:
                self.result = "ok"
                self.tag = ""
            elif oRs[0] == 1:
    ...
    def displayDeclaration(self, content):
        if self.request.GET.get("a", "") == "new":
            ...
            oRs = oConnExecute("SELECT id, tag, name, sp_alliance_war_cost(id) + (const_coef_score_to_war()*sp_alliance_value(" + str(self.AllianceId) + "))::integer FROM alliances WHERE lower(tag)=lower(" + strSql(self.tag) + ")")
            if oRs == None:
    ...
    def displayPage(self, cat):
        content = getTemplateContext(self.request, "alliance-wars")
