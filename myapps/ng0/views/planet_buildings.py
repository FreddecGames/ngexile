...
class View(BaseView):
    ...
    def RetrievePlanetInfo(self):
        ...
        query = "SELECT ore, hydrocarbon, workers-workers_busy, workers_capacity - workers, energy, " + \
                ...
                " FROM vw_planets" + \
                " WHERE id="+str(self.currentPlanet['id'])
        ...
        query = "SELECT planetid, buildingid, quantity FROM planet_buildings WHERE quantity > 0 AND planetid=" + str(self.currentPlanet['id'])
    ...
    def ListBuildings(self):
        ...
        oRs = oConnExecute("SELECT int4(count(*)) FROM planet_buildings_pending WHERE planetid=" + str(self.currentPlanet['id']) + " LIMIT 1")
        ...
        query = "SELECT id, category, cost_prestige, cost_ore, cost_hydrocarbon, cost_energy, cost_credits, workers, floor, space," + \
                ...
                " FROM vw_buildings" + \
                " WHERE planetid=" + str(self.currentPlanet['id']) + " AND ((buildable AND research_requirements_met) or quantity > 0)"
    
        oRss = oConnExecute(query)
        content = getTemplateContext(self.request, "planet-buildings")
    
        content.assignValue("planetid", str(self.currentPlanet['id']))
    ...
    def StartBuilding(self, BuildingId):
        oRs = connExecuteRetry("SELECT sp_start_building(" + str(self.profile['id']) + "," + str(self.currentPlanet['id']) + ", " + str(BuildingId) + ", false)")
        
    def CancelBuilding(self, BuildingId):
        connExecuteRetryNoRecords("SELECT sp_cancel_building(" + str(self.profile['id']) + "," + str(self.currentPlanet['id']) + ", " + str(BuildingId) + ")")
    
    def DestroyBuilding(self, BuildingId):
        connExecuteRetryNoRecords("SELECT sp_destroy_building(" + str(self.profile['id']) + "," + str(self.currentPlanet['id']) + "," + str(BuildingId) + ")")
