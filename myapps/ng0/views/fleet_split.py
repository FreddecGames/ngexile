...
class View(BaseView):
    ...
    def DisplayExchangeForm(self, fleetid):
        ...
        content = getTemplateContext(self.request, "fleet-split")
        ...
        query = "SELECT id, name, attackonsight, engaged, size, signature, speed, remaining_time, commanderid, commandername," + \
                ...
                " action " + \
                " FROM vw_fleets" + \
                " WHERE ownerid="+str(self.profile['id'])+" AND id="+str(fleetid)
    ...
    def SplitFleet(self, fleetid):
        ...
        #
        query = "SELECT planetid FROM vw_fleets WHERE ownerid="+str(self.profile['id'])+" AND id="+str(fleetid)
        oRs = oConnExecute(query)
        if oRs == None: return
        ...
        #
        oRs = oConnExecute("SELECT sp_create_fleet(" + str(self.profile['id']) + "," + str(fleetplanetid) + "," + strSql(newfleetname) + ")")
        if oRs == None:
            return
        ...
        dbQuery("UPDATE fleets SET idle_since=now()" + \
                        " WHERE ownerid =" + str(self.profile['id']) + " AND (id="+str(newfleetid)+" OR id="+str(fleetid)+")")
        ...
        oRs = oConnExecute("SELECT cargo_capacity FROM vw_fleets WHERE ownerid="+str(self.profile['id'])+" AND id="+str(newfleetid))
        if oRs == None:
                return
        ...
        if ore != 0 or hydrocarbon != 0 or scientists != 0 or soldiers != 0 or workers != 0:
            ...
            dbQuery("UPDATE fleets SET" + \
                        ...
                        " WHERE id =" + str(newfleetid) + " AND ownerid =" + str(self.profile['id']))
            ...
            dbQuery("UPDATE fleets SET" + \
                        ...
                        " WHERE id =" + str(fleetid) + " AND ownerid =" + str(self.profile['id']))
        ...
        query = "DELETE FROM fleets WHERE ownerid=" + str(self.profile['id']) + " AND size=0"
        dbQuery(query)
