...
class View(BaseView):
    ...
    def ListCommanders(self):
        content = getTemplateContext(self.request, "empire-commanders")

        content.assignValue("planetid", self.currentPlanet['id'])

        # generate new commanders if needed for the player
        oConnExecute("SELECT sp_commanders_check_new_commanders(" + str(self.profile['id']) + ")")

        # retrieve how many commanders are controled by the player
        oRs = oConnExecute("SELECT int4(count(1)) FROM commanders WHERE recruited <= now() AND ownerid=" + str(self.profile['id']))
        ...
        query = "SELECT c.id, c.name, c.recruited, points, added, salary, can_be_fired, " + \
                ...
                " WHERE c.ownerid=" + str(self.profile['id']) + \
    ...
    def DisplayCommanderEdition(self, CommanderId):
        ...
        if CommanderId != 0:
            ...
            query = "SELECT mod_production_ore, mod_production_hydrocarbon, mod_production_energy," + \
                    ...
                    " points, name" + \
                    " FROM commanders WHERE id=" + str(CommanderId) + " AND ownerid=" + str(self.profile['id'])
    ...
    def EditCommander(self, CommanderId):
        ...
        query = "UPDATE commanders SET" + \
                ...
                " WHERE ownerid=" + str(self.profile['id']) + " AND id=" + str(CommanderId)
        ...
        query = "SELECT mod_production_ore, mod_production_hydrocarbon, mod_production_energy," + \
                    ...
                    " mod_construction_speed_buildings, mod_construction_speed_ships" + \
                    " FROM commanders" + \
                    " WHERE id=" + str(CommanderId) + " AND ownerid=" + str(self.profile['id'])
        oRs = oConnExecute(query)
    ...
    def RenameCommander(self, CommanderId, NewName):
        ...
        query = "SELECT sp_commanders_rename(" + str(self.profile['id']) + "," + str(CommanderId) + "," + strSql(NewName) + ")"
        dbQuery(query)
        return HttpResponseRedirect("/s03/empire-commanders/")

    def EngageCommander(self, CommanderId):

        query = "SELECT sp_commanders_engage(" + str(self.profile['id']) + "," + str(CommanderId) + ")"
        dbQuery(query)
        return HttpResponseRedirect("/s03/empire-commanders/")

    def FireCommander(self, CommanderId):

        query = "SELECT sp_commanders_fire(" + str(self.profile['id']) + "," + str(CommanderId) + ")"
        dbQuery(query)
        return HttpResponseRedirect("/s03/empire-commanders/")

    def TrainCommander(self, CommanderId):

        query = "SELECT sp_commanders_train(" + str(self.profile['id']) + "," + str(CommanderId) + ")"
        dbQuery(query)
        return HttpResponseRedirect("/s03/empire-commanders/")
