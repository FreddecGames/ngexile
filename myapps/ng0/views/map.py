...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if planet != "":
            galaxy = self.currentPlanet['galaxy']
            sector = self.CurrentSectorId
        else:
            if galaxy != "": galaxy = toInt(galaxy,self.currentPlanet['galaxy'])
    ...
    def displayRadar(self, content, galaxy, sector, radarstrength):
        ...
        query = "SELECT v.id, v.name, attackonsight, engaged, size, signature, speed, remaining_time," + \
                ...
                " WHERE userid="+str(self.profile['id'])+" AND ("+ \
    ...
    def DisplayMap(self, galaxy, sector):
        ...
        content = getTemplateContext(self.request, "map")
        ...
        if galaxy == "":
            ...
            query = "SELECT n.id, "+ \
                    " n.colonies > 0,"+ \
                    " False AND EXISTS(SELECT 1 FROM nav_planet WHERE galaxy=n.id AND ownerid IN (SELECT friend FROM vw_friends WHERE vw_friends.userid="+str(self.profile['id'])+") LIMIT 1),"+ \
                    " EXISTS(SELECT 1 FROM nav_planet WHERE galaxy=n.id AND ownerid IN (SELECT ally FROM vw_allies WHERE vw_allies.userid="+str(self.profile['id'])+") LIMIT 1),"+ \
                    " EXISTS(SELECT 1 FROM nav_planet WHERE galaxy=n.id AND ownerid = "+str(self.profile['id'])+" LIMIT 1) AS hasplanets"+ \
        ...
        if sector == "":
            ...
            #
            query = "SELECT sp_get_galaxy_planets(" + str(galaxy) + "," + str(self.profile['id']) + ")"
            ...
            query = "SELECT sell_ore, sell_hydrocarbon FROM sp_get_resource_price(" + str(self.profile['id']) + "," + str(galaxy) + ", false)"
            oRs = oConnExecute(query)
        ...
        query = "SELECT f.planetid, f.id, f.name, sp_relation(f.ownerid, "+str(self.profile['id'])+"), f.signature," + \
                "    EXISTS(SELECT 1 FROM fleets AS fl WHERE fl.planetid=f.planetid and fl.action != 1 and fl.action != -1 and fl.ownerid IN (SELECT ally FROM vw_allies WHERE userid="+str(self.profile['id'])+") LIMIT 1)," + \
                " action=1 OR action=-1, (SELECT tag FROM alliances WHERE id=users.alliance_id), username, shared," + \
                "    EXISTS(SELECT 1 FROM fleets AS fl WHERE fl.planetid=f.planetid and fl.action != 1 and fl.action != -1 and fl.ownerid ="+str(self.profile['id'])+" LIMIT 1)" + \
                " FROM fleets as f" + \
        ...
        #
        query = "SELECT * FROM sp_get_user_rs("+str(self.profile['id'])+","+str(galaxy)+","+str(sector)+")"
        oRs = oConnExecute(query)
        radarstrength = oRs[0]
        ...
        query = "SELECT nav_planet.id, nav_planet.planet, nav_planet.name, nav_planet.ownerid,"+ \
                " users.username, sp_relation(nav_planet.ownerid," + str(self.profile['id']) + "), floor, space, GREATEST(0, radar_strength), radar_jamming," + \
