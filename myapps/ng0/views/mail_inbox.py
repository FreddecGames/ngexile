...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # reply
            ...
            query = "SELECT sender, subject, body FROM messages WHERE ownerid=" + str(self.profile['id']) + " AND id=" + str(Id) + " LIMIT 1"
        ...
        # send email
            ...
            if self.mailbody == "":
                ...
                if self.mailto == "":
                    ...
                    oRs = oConnExecute("SELECT sp_send_message("+ str(self.profile['id']) + "," + strSql(self.mailto) + "," + strSql(self.mailsubject) + "," + strSql(self.mailbody) + "," + str(self.moneyamount) + "," + str(self.bbcode) + ")")
        ...
        # delete selected emails
            ...
            if query != "False":
                dbQuery("UPDATE messages SET deleted=True WHERE (" + query + ") AND ownerid = " + str(self.profile['id']))
        
        if request.GET.get("a", "") == "ignore":
            print(strSql(request.GET.get("user")))
    ...
    def display_mails(self):
        ...
        content = getTemplateContext(self.request, "mail-inbox")
        ...
        query = "SELECT count(1) FROM messages WHERE NOT deleted AND ownerid = " + str(self.profile['id'])
        oRs = oConnExecute(query)
        size = int(oRs[0])
        ...
        query = "SELECT sender, subject, body, datetime, messages.id, read_date, avatar_url, users.id, messages.credits," + \
                ...
                "    LEFT JOIN messages_ignore_list ON (userid=" + str(self.profile['id']) + " AND ignored_userid = users.id)" + \
                " WHERE NOT deleted AND ownerid = " + str(self.profile['id']) + \
        ...
        if not self.request.user.is_impersonate:
            oRs = dbQuery("UPDATE messages SET read_date = now() WHERE ownerid = " + str(self.profile['id']) + " AND read_date IS NULL" )
    ...
    def display_mails_sent(self):
        ...
        content = getTemplateContext(self.request, "mail-outbox")
        ...
        # get total number of mails that could be displayed
        query = "SELECT count(1) FROM messages WHERE "+messages_filter+"senderid = " + str(self.profile['id'])
        oRs = oConnExecute(query)
        size = int(oRs[0])
        nb_pages = int(size/displayed)
        if nb_pages*displayed < size: nb_pages = nb_pages + 1
        ...
        query = "SELECT messages.id, owner, avatar_url, datetime, subject, body, messages.credits, users.id, bbcode, alliances.tag"+ \
                ...
                "    LEFT JOIN alliances ON (users.alliance_id = alliances.id)" + \
                " WHERE "+messages_filter+"senderid = " + str(self.profile['id']) + \
    ...
    def display_compose_form(self, mailto, subject, body, credits):
        ...
        content = getTemplateContext(self.request, "mail-new")

        # fill the recent addressee list

        oRss = oConnExecuteAll("SELECT * FROM sp_get_addressee_list(" + str(self.profile['id']) + ")")
        ...
        oRs = oConnExecute("SELECT credits, now()-game_started > INTERVAL '2 weeks' AND security_level >= 3 FROM users WHERE id="+str(self.profile['id']))
