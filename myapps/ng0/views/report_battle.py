...
class View(GetView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if battleId == 0: return HttpResponseRedirect('/ng0/reports/')
        
        fromview = ToInt(request.GET.get('v'), 0)
        if fromview == 0: return HttpResponseRedirect('/ng0/reports/')
        ...
        if not display:
            ...
            if key == "": return HttpResponseRedirect("/ng0/reports/")
            result = dbExecute("SELECT 1 FROM battles WHERE id=" + str(battleId) + " AND key=" + dosql(key))
            display = result != None
            
        if not display: return HttpResponseRedirect("/ng0/reports/")            
            
        #---
        
        self.tpl = getTemplate(request, 'ng0/battle')
    
        self.selectedMenu = 'reports'
        
        #---
        
        self.tpl.setValue('fromview', fromview)
        ...
        battle = dbRow(query)
        self.tpl.setValue('battle', battle)
        ...
        self.tpl.setValue('baseurl', request.META.get('HTTP_HOST'))
