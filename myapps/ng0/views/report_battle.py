...
class View(BaseView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if id == 0:
            return HttpResponseRedirect("/s03/empire-reports/")

        creator = self.profile['id']

        fromview = toInt(request.GET.get("v"), self.profile['id'])

        display_battle = True

        # check that we took part in the battle to display it
        oRs = oConnExecute("SELECT battleid FROM battles_ships WHERE battleid=" + str(id) + " AND owner_id=" + str(self.profile['id']) + " LIMIT 1")
        ...
        if display_battle:
            ...
            content = self.FormatBattle(id, creator, fromview, False)
    ...
    def FormatBattle(self, battleid, creator, pointofview, ispubliclink):
        ...
        content = getTemplateContext(self.request, "report-battle")
        ...
        if not ispubliclink:
            ...
            content.assignValue("baseurl", self.request.META.get("HTTP_HOST"))
