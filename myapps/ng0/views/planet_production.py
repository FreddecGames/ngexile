...
class View(BaseView):
    ...
    def displayOverview(self, RecomputeIfNeeded):
        ...
        query = "SELECT id, production_ore*working_quantity, production_hydrocarbon*working_quantity, energy_production*working_quantity, working_quantity"+ \
                " FROM vw_buildings" + \
                " WHERE planetid="+str(self.currentPlanet['id'])+" AND (production_ore > 0 OR production_hydrocarbon > 0 OR energy_production > 0) AND working_quantity > 0;"
        ...
        query = "SELECT commanders.id, commanders.name," + \
                ...
                " WHERE nav_planet.id=" + str(self.currentPlanet['id'])
        ...
        query = "SELECT buildingid, '', mod_production_ore*quantity, mod_production_hydrocarbon*quantity, mod_production_energy*quantity" + \
                ...
                " WHERE planetid="+str(self.currentPlanet['id'])+" AND (mod_production_ore != 0 OR mod_production_hydrocarbon != 0 OR mod_production_energy != 0)"
        ...
        query = "SELECT researchid, '', level*mod_production_ore, level*mod_production_hydrocarbon, level*mod_production_energy, level" + \
                ...
                " WHERE userid=" + str(self.profile['id']) +" AND ((mod_production_ore > 0) OR (mod_production_hydrocarbon > 0) OR (mod_production_energy > 0)) AND (level > 0);"
        ...
        query = "SELECT int4(COALESCE(sum(effective_energy), 0)) FROM planet_energy_transfer WHERE target_planetid=" + str(self.currentPlanet['id'])
        oRs = oConnExecute(query)
        EnergyReceived = oRs[0]

        # Assign total production variables
        query = "SELECT ore_production, hydrocarbon_production, energy_production-"+str(EnergyReceived)+" FROM nav_planet WHERE id=" + str(self.currentPlanet['id'])
        oRs = oConnExecute(query)
    ...
    def displayReceiveSendEnergy(self):
        ...
        query = "SELECT energy_receive_antennas, energy_send_antennas FROM nav_planet WHERE id=" + str(self.currentPlanet['id'])
        oRs = oConnExecute(query)
        ...
        if self.action == "cancel":
            ...
            if energy_from != 0:
                query = "DELETE FROM planet_energy_transfer WHERE planetid=" + str(energy_from) + " AND target_planetid=" + str(self.currentPlanet['id'])
            else:
                query = "DELETE FROM planet_energy_transfer WHERE planetid=" + str(self.currentPlanet['id']) + " AND target_planetid=" + str(energy_to)
            ...
            for oRs in oRss:
                ...
                if query != "":
                    query = "UPDATE planet_energy_transfer SET " + query + " WHERE planetid=" + str(self.currentPlanet['id']) + " AND target_planetid=" + str(oRs[0])
                    dbQuery(query)
            ...
            if g != 0 and s != 0 and p != 0 and energy > 0:
                query = "INSERT INTO planet_energy_transfer(planetid, target_planetid, energy) VALUES(" + str(self.currentPlanet['id']) + ", sp_planet(" + str(g) + "," + str(s) + "," + str(p) + ")," + str(energy) + ")"
                dbQuery(query)

                update_planet = True

            if update_planet:
                query = "SELECT sp_update_planet(" + str(self.currentPlanet['id']) + ")"
                dbQuery(query)

            return HttpResponseRedirect("/s03/planet-production/?cat=" + str(self.cat))

        query = "SELECT t.planetid, sp_get_planet_name(" + str(self.profile['id']) + ", n1.id), sp_relation(n1.ownerid," + str(self.profile['id']) + "), n1.galaxy, n1.sector, n1.planet, " + \
                "        t.target_planetid, sp_get_planet_name(" + str(self.profile['id']) + ", n2.id), sp_relation(n2.ownerid," + str(self.profile['id']) + "), n2.galaxy, n2.sector, n2.planet, " + \
                ...
                " WHERE planetid=" + str(self.currentPlanet['id']) + " OR target_planetid=" + str(self.currentPlanet['id']) + \
        ...
        for oRs in oRss:
            ...
            if oRs[0] == self.currentPlanet['id']:
                sending = sending + 1
    ...
    def displayPage(self, RecomputeIfNeeded):
        ...
        self.content = getTemplateContext(self.request, "planet-production")
