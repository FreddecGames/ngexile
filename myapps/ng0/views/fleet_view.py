...
class View(GetPostView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.fleetId == 0: return HttpResponseRedirect("/ng0/fleets/")
        ...
        if not self.fleetOwnerId: return HttpResponseRedirect("/ng0/fleets/")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'invade':
            ...
            if result > 0: return HttpResponseRedirect("/ng0/invasion/?id=" + str(result) + "&fleetid=" + str(self.fleetId))
            ...
            return HttpResponseRedirect('/ng0/fleet/?id=' + str(self.fleetId))
    ...
    def get(self, request, *args, **kwargs):
        ...
        self.tpl = getTemplate(request, "ng0/fleet")
        
        self.selectedMenu = "fleets"
        
        self.headerUrl = '/ng0/orbit/'
        ...
        self.tpl.setValue("fleet", fleet)

        #---

        if self.allianceId and fleet['shared']: self.tpl.Parse("shared")
        ...
        if fleet['action'] == -1 or fleet['action'] == 1:
            ...
            if not fleet['engaged'] and fleet['total_time'] - fleet['remaining_time'] < timelimit and fleet['planetid']:
                self.tpl.setValue("timelimit", timelimit - (fleet['total_time'] - fleet['remaining_time']))
                self.tpl.Parse("can_cancel_moving")

        elif not fleet['engaged'] and fleet['action'] == 0:

            self.tpl.Parse("can_move")
            
            #---
            
            if fleet['warp_to']: self.tpl.Parse("can_warp")
            
            if fleet['recycler_output'] > 0 and (fleet['orbit_ore'] or fleet['orbit_hydrocarbon'] or fleet['spawn_ore'] or fleet['spawn_hydrocarbon']): self.tpl.Parse("can_recycle")

            if ((fleet['planet_ownerid'] == None) or (fleet['planet_owner_relation'] >= rHostile)) and (fleet['warp_to'] == None): self.tpl.Parse("can_install_building")
            
            if fleet['cargo_load'] > 0 and fleet['planet_owner_relation'] >= rFriend: self.tpl.Parse("can_unload_cargo")
            
            if fleet['cargo_capacity'] > 0 and fleet['planet_owner_relation'] == rSelf:
            
                self.tpl.Parse("can_load_cargo")
                ...
                planet = dbRow(query)
                self.tpl.setValue("planet", planet)
                
            if fleet['planet_ownerid'] == self.userId: self.tpl.Parse("can_manage")
                
            if fleet['size'] > 1 and self.fleetOwnerId == self.userId: self.tpl.Parse("can_split")                

            if fleet['planet_ownerid'] and fleet['planet_owner_relation'] < rFriend and fleet['droppods'] > 0:

                self.tpl.Parse("can_invade")
            
                if fleet['idle_time'] < fleet['before_invasion']: t = fleet['before_invasion'] - fleet['idle_time']
                else: t = 0 
                self.tpl.setValue("invade_time", int(t))
            ...
            self.tpl.setValue("planetgroup", planetgroup)
            ...
            self.tpl.setValue("fleetgroup", fleetgroup)
            ...
            self.tpl.setValue("merchantplanetsgroup", merchantplanetsgroup)
        ...
        if fleet['routeid']:
            ...
            self.tpl.setValue("actions", actions)
        ...
        cmd_groups = []
        self.tpl.setValue('optgroups', cmd_groups)
