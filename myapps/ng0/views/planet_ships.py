...
class View(BaseView):
    ...
    def ListShips(self):
        ...
        query = "SELECT id, category, name, cost_ore, cost_hydrocarbon, cost_energy, workers, crew, capacity," + \
                ...
                " FROM vw_ships WHERE planetid=" + str(self.currentPlanet['id'])
        ...
        content = getTemplateContext(self.request, "planet-ships")
        ...
        self.displayQueue(content, str(self.currentPlanet['id']))
    ...
    def ListRecycleShips(self):
        ...
        query = "SELECT id, category, name, int4(cost_ore * const_recycle_ore(planet_ownerid)) AS cost_ore, int4(cost_hydrocarbon * const_recycle_hydrocarbon(planet_ownerid)) AS cost_hydrocarbon, cost_credits, workers, crew, capacity," + \
                ...
                " FROM vw_ships" + \
                " WHERE quantity > 0 AND planetid=" + str(self.currentPlanet['id'])

        oRss = dbRows(query)
        
        content = getTemplateContext(self.request, "shipyard-recycle")
    ...
    def StartShip(self, ShipId, quantity):
        connExecuteRetryNoRecords("SELECT sp_start_ship(" + str(self.currentPlanet['id']) + "," + str(ShipId) + "," + str(quantity) + ", false)")
    ...
    def RecycleShip(self, ShipId, quantity):
        connExecuteRetryNoRecords("SELECT sp_start_ship_recycling(" + str(self.currentPlanet['id']) + "," + str(ShipId) + "," + str(quantity) + ")")
    ...
    def CancelQueue(self, QueueId):
        connExecuteRetryNoRecords("SELECT sp_cancel_ship(" + str(self.currentPlanet['id']) + ", " + str(QueueId) + ")")
