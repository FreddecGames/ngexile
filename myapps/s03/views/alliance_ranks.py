...
class View(GlobalView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for rank in ranks:
            ...
            name = self.request.POST.get('n' + str(rank['rankid'])).strip()
            if len(name) > 2:
            
                query = 'UPDATE alliances_ranks SET' + \
                        ' label=' + dosql(name) + \
                        ', is_default=NOT leader AND ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_0'), False)) + \
                        ', can_invite_player=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_1'), False)) + \
                        ', can_kick_player=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_2'), False)) + \
                        ', can_create_nap=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_3'), False)) + \
                        ', can_break_nap=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_4'), False)) + \
                        ', can_ask_money=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_5'), False)) + \
                        ', can_see_reports=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_6'), False)) + \
                        ', can_accept_money_requests=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_7'), False)) + \
                        ', can_change_tax_rate=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_8'), False)) + \
                        ', can_mail_alliance=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_9'), False)) + \
                        ', can_manage_description=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_10'), False)) + \
                        ', can_manage_announce=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_11'), False)) + \
                        ', can_see_members_info=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_12'), False)) + \
                        ', members_displayed=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_13'), False)) + \
                        ', can_order_other_fleets=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_14'), False)) + \
                        ', can_use_alliance_radars=leader OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_15'), False)) + \
                        ', enabled=leader OR EXISTS(SELECT 1 FROM users WHERE alliance_id=' + str(self.allianceId) + ' AND alliance_rank=' + str(rank['rankid']) + ' LIMIT 1) OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_enabled'), False)) + ' OR ' + str(ToBool(self.request.POST.get('c' + str(rank['rankid']) + '_0'), False)) + \
    ...
    def get(self, request, *args, **kwargs):
        ...
        content = getTemplate(self.request, 's03/alliance-ranks')
